...
M2：二维俯视据点与初始伙伴本地原型已实现。登录后可移动、与 NPC 对话、三选一领取初始伙伴，刷新或重登后恢复队伍。已有账号和基础档案保留；战斗、野外、多人玩法和公开部署尚未实现。

本轮采用 Node.js 内置模块与 SQLite，不安装第三方依赖。未来游戏引擎与多人模式仍待确认；当前选择与边界见[首个实现里程碑](docs/architecture/account-save-milestone.md)。
