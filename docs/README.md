...
[底层总览](architecture/foundation-design.md)、[核心玩法规则](game-design/core-rules.md)、[1.0 内容大纲](versions/1.0/outline.md)共同收敛第一条可玩流程。
...
账号认证、会话和恢复写进账号专题；永久数据结构、事务和备份写进数据专题；1.0 只引用它们并定义交付范围。具体登录页面或初始宠物选择流程确定后，再补玩家体验设计，不复制认证规则。
